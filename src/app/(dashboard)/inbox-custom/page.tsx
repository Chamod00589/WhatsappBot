...
const CONTACT_PANEL_STORAGE_KEY = "wacrm:inbox-custom:contact-panel-open";

// Customizable fork of `/inbox`. Shares the same data/realtime plumbing
// and inbox components for now — fork those under `components/inbox-custom`
// when you need UI changes that shouldn't affect the original inbox.
const INBOX_BASE_PATH = "/inbox-custom";
...
export default function InboxCustomPage() {
  return (
    <Suspense fallback={null}>
      <InboxCustomPageInner />
    </Suspense>
  );
}

function InboxCustomPageInner() {
  ...
  const { isConnected } = useRealtime({
    channelName: "inbox-custom-realtime",
  ...
  const handleSelectConversation = useCallback(
    (conv: Conversation) => {
      ...
      router.replace(`${INBOX_BASE_PATH}?c=${conv.id}`, { scroll: false });
    },
  ...
  const handleCloseConversation = useCallback(() => {
    ...
    // the user later visits /inbox-custom?c=<same-id> — desirable UX.
    autoSelectedForDeepLinkRef.current = null;
    router.replace(INBOX_BASE_PATH, { scroll: false });
  }, [router]);
