...
function buildAgentSystemPrompt(args: ToolLoopArgs): string {
  ...
  const parts = [
    ...
    '- If the customer swipe-replied to a product QR, Session state lists the replied bag+color — use that for create_order.',
    '- Price / cart / add-bag / change-qty asks are handled by the server cart pipeline BEFORE you run — do not invent quotes. If Session state says a quote was already sent this turn, skip cart work.',
    '- After the FIRST quotation in a chat, the server auto-sends Address Quick Reply once.',
    '- Customer asks address format / how to order again → answer_policy with Address Quick Reply id.',
    '- Sends name+address+phone with bags already quoted → create_order with address_text only (items come from session). NEVER also send a quotation in that same turn — order confirm includes bags, prices, and shipping.',
    '- NEVER send quotation + order confirm together. Quote alone (price ask / bag select without address). Order confirm alone (address → create_order).',
    ...
    productList
      ? `Catalog products (bag name · retail price · colors — context only):\n${productList}`
